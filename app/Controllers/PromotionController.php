...
class PromotionController extends BaseController
{
    ...
    public function create_promotion()
    {
        ...
        if ($type_promotion == '1' || $type_promotion == '2') {
            $id_book_cat = $this->request->getVar('id_book_cat');
        } else if ($type_promotion == '3' || $type_promotion == '4') {
            $id_book_cat = $this->request->getVar('id_book_cat_2');
        }
        $data = [
            'details' => $this->request->getVar('detail_promotion'),
            'type_promotion' => $type_promotion,
            'id_book_cat' => $id_book_cat,
    ...
    public function cal_promotion()
    {
        ...
        if ($sum_id_book_array[0] != '') {
                ...
                foreach ($data['promotion'] as $key_promotion => $value_promotion) {
                    ...
                    if ($value_promotion['status'] == '1') {
                        ...
                        if ($value_promotion['type_sale'] == '1') {
                            ...
                            //เช็คว่าประเภท ลดจากหนังสือหรือลดจากหมวดหมู่ [1] หนังสือ [2]หมวดหมู่ [3] เช่าครบ /ครั้ง [4] บิลเช่ามากกว่าหรือเท่า /เล่ม
            ...
            foreach ($data['promotion'] as $key_promotion => $value_promotion) {
                if ($value_promotion['status'] == '1') {
                    // เช็คว่าประเภท การลดคิดแบบไหน [1] - [2] %
                    if ($value_promotion['type_sale'] == '1') {
                        if ($value_promotion['type_promotion'] == '3') {
                            if ($countHis >= $value_promotion['id_book_cat']) {
                                $text .= $value_promotion['details'] . '<br>';
                                $sumid_promotion .= $value_promotion['id_promotion'] . ',';
                                $price_promotion += $value_promotion['number_cal'];
                            }
        ...
        $integer_value = intval($price_promotion);

        $response = [
            'price_result' => $sum_price,
            'price_promotion' => $integer_value,
